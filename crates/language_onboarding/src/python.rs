                ...
                el.child(
                    Banner::new()
                        .child(
                            v_flex()
                                .gap_0p5()
                                .child(Label::new(locale::t_static("Basedpyright is now the only default language server for Python")).mt_0p5())
                                .child(Label::new(locale::t_static("We have disabled PyRight and pylsp by default. They can be re-enabled in your settings.")).size(LabelSize::Small).color(Color::Muted))
                        )
                        .action_slot(
                            h_flex()
                                .gap_0p5()
