...
impl Item for KeyContextView {
    ...
    fn tab_content_text(&self, _detail: usize, _cx: &App) -> SharedString {
        locale::t_static("Keyboard Context")
    }
...
impl Render for KeyContextView {
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl ui::IntoElement {
        ...
        v_flex()
            ...
            .child(Label::new(locale::t_static("Keyboard Context")).size(LabelSize::Large))
            .child(Label::new(locale::t_static("This view lets you determine the current context stack for creating custom key bindings in Zed. When a keyboard shortcut is triggered, it also shows all the possible contexts it could have triggered in, and which one matched.")))
            .child(
                h_flex()
                    .mt_4()
                    .gap_4()
                    .child(
                        Button::new("open_documentation", locale::t_static("Open Documentation"))
                            .style(ButtonStyle::Filled)
                            .on_click(|_, _, cx| cx.open_url("https://zed.dev/docs/key-bindings")),
                    )
                    .child(
                        Button::new("view_default_keymap", locale::t_static("View Default Keymap"))
                            .style(ButtonStyle::Filled)
                    ...
                    .child(
                        Button::new("edit_your_keymap", locale::t_static("Edit Keymap File"))
                            .style(ButtonStyle::Filled)
            ...
            .child(
                Label::new(locale::t_static("Current Context Stack"))
            ...
            .child(Label::new(locale::t_static("Last Keystroke")).mt_4().size(LabelSize::Large))
            ...
            .when_some(key_equivalents, |el, key_equivalents| {
                el.child(Label::new(locale::t_static("Key Equivalents")).mt_4().size(LabelSize::Large))
                    .child(Label::new(locale::t_static("Shortcuts defined using some characters have been remapped so that shortcuts can be typed without holding option.")))
                    .children(
                        key_equivalents
                            .iter()
                            .sorted()
