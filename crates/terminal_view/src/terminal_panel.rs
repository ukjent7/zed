        ...
        let popover_menu = PopoverMenu::new("settings-popover")
            ...
            .menu(move |window, cx| {
                Some(ContextMenu::build(window, cx, |context_menu, _, _| {
                    context_menu
                        .action(
                            locale::t("Open Settings"),
                            zed_actions::OpenSettings.boxed_clone(),
                        )
                        .action(
                            locale::t("Edit settings.json"),
                ...
                v_flex()
                    ...
                    .child(SplitButton::new(
                        ButtonLike::new("open-settings-ui")
                            .child(Label::new(locale::t("Edit Settings")).size(LabelSize::Small))
    ...
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let restoring_placeholder = (waiting_for_terminals && no_items_in_panes).then(|| {
            let label = if self.restoring {
                locale::t("Restoring terminals…")
            } else {
                locale::t("Starting terminal…")
            };
            h_flex()
                .absolute()
                .inset_0()
